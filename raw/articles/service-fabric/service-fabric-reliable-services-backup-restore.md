...
Azure Service Fabric is a high-availability platform that replicates the state across multiple nodes to maintain this high availability. Thus, even if one node in the cluster fails, the services continue to be available. While this in-built redundancy provided by the platform may be sufficient for some, in certain cases it's desirable for the service to back up data (to an external store).

> [!NOTE]
> It's critical to backup and restore your data (and test that it works as expected) so you can recover from data loss scenarios.
> 

> [!NOTE]
> Microsoft recommends using [Periodic backup and restore](service-fabric-backuprestoreservice-quickstart-azurecluster.md) for configuring data backup of Reliable Stateful services and Reliable Actors. 
> 
...
For example, a replica that has 16 GB of state has checkpoints that add up approximately to 16 GB.
...
Since incremental backups are only changes since the last backup (doesn't include the checkpoints), they tend to be faster but they can't be restored on their own.
...
The service author has full control of when to make backups and where backups are stored.
...
Note that:
  ...
  - There can be only one backup operation in-flight per replica at any given time. More than one `BackupAsync` call at a time throws `FabricBackupInProgressException` to limit inflight backups to one.
  - If a replica fails over while a backup is in progress, the backup may not have been completed. Thus, once the failover finishes, it's the service's responsibility to restart the backup by invoking `BackupAsync` as necessary.
...
In general, the cases when you might need to perform a restore operation fall into one of these categories:
  ...
  - The service replicated corrupt application data (for example, because of an application bug). In this case, the service has to be upgraded or reverted to remove the cause of the corruption, and noncorrupt data has to be restored.
...
`RestoreAsync` call can throw `FabricMissingFullBackupException` if the `BackupFolderPath` provided doesn't contain a full backup.
...
Note that:
  ...
  - When you restore, there's a chance that the backup being restored is older than the state of the partition before the data was lost. Because of this, you should restore only as a last resort to recover as much data as possible.
...
If the `OnDataLossAsync` returns true, Service Fabric rebuilds all other replicas from this primary. Service Fabric ensures that replicas that receive `OnDataLossAsync` call first transition to the primary role but aren't granted read status or write status.
This implies that for StatefulService implementers, `RunAsync` won't be called until `OnDataLossAsync` finishes successfully.
Then, `OnDataLossAsync` is invoked on the new primary.
Until a service completes this API successfully (by returning true or false) and finishes the relevant reconfiguration, the API keeps being called one at a time.
