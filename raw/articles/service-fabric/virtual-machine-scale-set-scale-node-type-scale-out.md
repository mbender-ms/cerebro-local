...
ms.date: 03/22/2026
# Customer intent: As a cloud administrator, I want to add a new node type to an existing Service Fabric cluster, so that I can scale my application resources horizontally without downtime, ensuring optimal performance and availability for running workloads.
---
...
Adding a new node type (which is backed by a virtual machine scale set) to an existing cluster is similar to [upgrading of the primary node type](service-fabric-scale-up-primary-node-type.md), except you won't use the same NodeTypeRef; obviously won't be disabling any actively used virtual machine scale sets, and you won't lose cluster availability if you don't update the primary node type. 
...
Additionally you need to add this new node type to your Service Fabric cluster resource:
...
## Next steps
* Learn how to [scale up the primary node type](service-fabric-scale-up-primary-node-type.md)
