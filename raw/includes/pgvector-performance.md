...
There are three supported index types:
* [Inverted File with Flat Compression (IVFFlat)](#inverted-file-with-flat-compression-ivfflat)
* [Hierarchical Navigable Small Worlds (HNSW)](#hierarchical-navigable-small-worlds-hnsw)
* [Disk Approximate Nearest Neighbor (DiskANN)](#disk-approximate-nearest-neighbor-diskann)

The `IVFFlat` index has faster build times and uses less memory than `HNSW`, but has lower query performance (in terms of speed-recall tradeoff). `DiskANN` offers a great balance of both offering highly accurate query performance and fast build times.
...
> [!NOTE]
> DiskANN is only supported in [Azure PostgreSQL Flexible Server](/azure/postgresql/flexible-server/how-to-use-pgdiskann).
